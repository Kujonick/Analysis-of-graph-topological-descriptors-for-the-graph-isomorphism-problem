...
if CHOSEN_FEATURES:
    ...
    node_features = [
        (f,)
        for f in list(filter(lambda k: "ldp" not in k, node_descriptors_dict.keys()))
    ]
    node_features.extend(
        [
            tuple(
                filter(
                    lambda k: "ldp" in k and "normalized" not in k,
                    node_descriptors_dict.keys(),
                )
            )
        ]
    )
    node_features.extend(
        [
            tuple(
                filter(
                    lambda k: "ldp" in k and ("normalized" in k or "degree" in k),
                    node_descriptors_dict.keys(),
                )
            )
        ]
...
if K_GRAPH2:
    all_features.extend(
        list(
            map(
                lambda params: tuple(map(lambda x: x + ":k_graph2", params)),
                base_features,
            )
        )
    )
if K_GRAPH3:
    all_features.extend(
        list(
            map(
                lambda params: tuple(map(lambda x: x + ":k_graph3", params)),
                base_features,
            )
        )
    )
if MODK_GRAPH3:
    all_features.extend(
        list(
            map(
                lambda params: tuple(map(lambda x: x + ":modk_graph3", params)),
                base_features,
            )
        )
    )
if COMBINED_K_GRAPH3:
    all_features.extend(
        list(
            map(
                lambda features: tuple(
                    x + k_graph
                    for k_graph in ["", ":k_graph2", ":k_graph3"]
                    for x in features
                ),
                base_features,
            )
...
if BENCHMARK_BREC:
    ...
    dataset_names = list(
        map(
            ...
            filter(
                lambda x: ".g6" in x and "brec" not in x,
                os.listdir(Settings.raw_datasets_dir),
            ),
        )
    )
for dataset_name in dataset_names:
    ...
    for features in all_features:
        ...
        arguments_list.append(
            TestParameters(dataset_name=dataset_name, features=features)
        )
